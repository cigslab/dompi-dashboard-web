...
const context={document:{body,createElement:tag=>new Element(tag)}, URLSearchParams,
...
(async()=>{
  ...
  close.events.click(); assert.equal(dialog.removed,true);
  ...
  assert.match(source,/openCategoryTransactions\(item.category, \{start: data.start, end: data.end\}\)/);
