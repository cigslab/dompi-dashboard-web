...
(async()=>{
  ...
  assert.equal(el('Features').hidden,true);assert.equal(el('UpgradeAction').hidden,false);checks++;
  ...
  for(const feature of ['Export','Advanced']) assert.equal(el(feature+'Status').textContent,'Belum tersedia');checks++;
  ...
  assert.equal(el('ReceiptStatus').textContent,'Aktif');checks++;
