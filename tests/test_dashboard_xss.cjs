...
async function runTypeReviewTests() {
  ...
  try {
    ...
    for(let i=0;i<50&&!document.querySelector('.type-review-modal article');i++)await wait();
    const dialog=document.querySelector('.type-review-modal');
    check(dialog.open,'dialog open');
    check(dialog.querySelector('h2').textContent==='Jenis transaksi perlu diperiksa','modal title');
    check(dialog.querySelector('.type-review-badge').textContent==='Pengeluaran','type badge');
    const buttons=[...dialog.querySelectorAll('.type-review-actions button')];
    check(buttons.every(b=>b.getBoundingClientRect().height>=44),'touch targets');
    if(innerWidth<=420)check(buttons.every(b=>Math.abs(b.getBoundingClientRect().width-dialog.querySelector('.type-review-actions').getBoundingClientRect().width)<1),'stacked full width');
    ...
    for(let i=0;i<50&&dialog.querySelector('article');i++)await wait();
    check(!dialog.querySelector('article'),'confirmed item removed');
    check(dialog.textContent.includes('Tidak ada transaksi yang perlu diperiksa'),'empty state');
    check(dialog.querySelector('.type-review-pagination').hidden,'single page pagination hidden');
    check(dialog.querySelector('.type-review-close').textContent==='×','close icon');
    check(dialog.querySelector('.type-review-close').getAttribute('aria-label'),'accessible close');
    dialog.querySelector('.type-review-close').click();
    for(let i=0;i<50&&document.querySelector('.type-review-modal');i++)await wait();
    check(!document.querySelector('.type-review-modal'),'icon closes modal');
